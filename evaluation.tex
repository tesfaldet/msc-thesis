...
target are reliably preserved in the synthesized result. \edit{The}{For example, the}
...
rippling flow of \path{water_3}\highlight{, are all captured in the synthesized results.}
...
This \highlight{approach} corresponds with attempting to mimic the optical flow 
...
This \highlight{result} is not unexpected and is a fundamental limitation of the 
...
This \highlight{pattern} was evident in the user study where the best-performing 
...
transfer that was demonstrated above. This \highlight{capability} was enabled by the factored
...
\highlight{Although the final two-stream model was statistically better than the baseline model, users were still able to reliably tell the difference between real and synthesized for longer exposure times. It is possible that for longer exposure times, spatial artifacts (\eg, chromatic aberration and noise) of the synthesized texture may have served as cues for distinguishing between real and synthesized. These artifacts suggest room for improvement on the representation of appearance and has been left for future work.}

Finally, a qualitative comparison with two extant methods for dynamic texture synthesis was performed. Overall, the results from the proposed two-stream model were shown to be qualitatively better than the compared methods, showing more temporal coherence and similarity in dynamics as well as fewer spatial and temporal artifacts. Furthermore, \edit{the other methods were shown to lack the capability of the novel dynamics style transfer shown by the proposed two-stream model}{it is unclear how the other methods can be applied to the novel dynamics style transfer, as can the proposed two-stream model}.